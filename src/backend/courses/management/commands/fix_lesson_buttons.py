    ...
    def handle(self, *args, **options):
        ...
        for lesson in Lesson.objects.all():
            ...
            if last_block.block_type == "button_next":
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Lesson {lesson.id}: Changed last block to button_next"
                    )
                )
            else:
                # Добавить новый блок button_next
                max_order = (
                    blocks.aggregate(max_order=models.Max("order"))["max_order"] or 0
                )
                ...
                added += 1
                self.stdout.write(
                    self.style.SUCCESS(f"Lesson {lesson.id}: Added button_next block")
                )
