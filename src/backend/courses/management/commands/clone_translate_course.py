...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE(f'Course created: {clone.title} (ID={clone.id})'))
        ...
        for group in original.group_set.all().order_by('order'):
            ...
            for module in group.module_set.all().order_by('order'):
                mod_clone = Module.objects.create(
                    ...
                    description=self.translate_text(translator, module.description or ''),
                    order=module.order
                )
                self.stdout.write(f'    Module cloned: {mod_clone.title} (ID={mod_clone.id})')
                ...
                for lesson in module.lesson_set.all().order_by('order'):
                    ...
                    self.stdout.write(f'      Lesson cloned: {lesson_clone.title} (ID={lesson_clone.id})')
                    ...
                    for block in lesson.contentblock_set.all().order_by('order'):
                        block_clone = ContentBlock.objects.create(
                            ...
                            content_html=self.translate_html(translator, block.content_html or ''),
                            content_text=self.translate_text(translator, block.content_text or ''),
                            content_json=block.content_json,
                            block_type=block.block_type,
                            order=block.order
                        )
                        self.stdout.write(f'        ContentBlock cloned: {block_clone.uuid} (type={block_clone.block_type})')

        self.stdout.write(self.style.SUCCESS('Course cloned and translated with detailed logging.'))