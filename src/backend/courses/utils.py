...
from openai_chats.models import Chat, ChatMessage
...
def process_lesson_to_json(text: str) -> dict:
    ...
    button_pattern = re.compile(
        r".*кнопка:.*[“\"'(\[\{«](.*?)[”\"'\)\]\}»].*$", re.IGNORECASE
    )
    input_prompt_pattern = re.compile(r".*инпут.*:.*$", re.IGNORECASE)
    ...
    for line in lines:
        ...
        if button_match:
            ...
            blocks.append(
                {"block_type": "button_continue", "content_text": button_match.group(1)}
            )
            continue
...
def get_chat_messages(user, input_block_uuid) -> list:
    ...
    user_chat = Chat.objects.filter(user=user, content_block=input_block).first()

    # Filter messages by the current user if you need to separate dialogs by users
    messages = ChatMessage.objects.filter(chat=user_chat).order_by("created_at")
