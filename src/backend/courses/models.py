...
class Module(CoreModel, SortableMixin):
    ...
    title = models.CharField(
        max_length=255,
        help_text="the special character `$` will be replaced by a sequence number",
    )
    description = models.TextField(blank=True, null=True)
    order = models.PositiveIntegerField(default=0, editable=False, db_index=True)
...
class Lesson(CoreModel, SortableMixin):
    ...
    duration = models.CharField(max_length=50, blank=True, null=True, default="15 min")
    ...
    def get_progress(self, user):
        progress = UserLessonProgress.objects.filter(user=user, lesson=self).first()
        if not progress:
            return 0
    ...
    async def aget_prompt(self, user) -> str:
        ...
        result = {
            ...
            "current_block": {"content_html": curr_blk.content_html or ""}
            if curr_blk
            else None,
            "next_block": {"content_html": next_blk.content_html or ""}
            if next_blk
            else None,
        }
...
class Access(models.Model):
    ...
    name = models.CharField(max_length=255, help_text="Название пакета доступа")
    description = models.TextField(
        blank=True, null=True, help_text="Описание того, что входит в пакет"
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="Когда пакет был создан"
    )
    updated_at = models.DateTimeField(
        auto_now=True, help_text="Когда пакет был в последний раз обновлён"
    )
