...
from payments.models import CloudPaymentOptions, Product, CloudPaymentTransaction
...
class CloudPaymentsWebhookView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not tx_id:
            return Response({"detail": "Missing TransactionId"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if amount != product.price or currency != product.currency:
            ...
            return Response({"detail": "Amount or currency mismatch"}, status=status.HTTP_400_BAD_REQUEST)
