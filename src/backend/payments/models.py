...
class Product(models.Model):
    ...
    name = models.CharField(
        max_length=255, help_text="Human-readable title of the product"
    )
    description = models.TextField(
        blank=True, null=True, help_text="Optional detailed description"
    )
    price = models.DecimalField(
        max_digits=10, decimal_places=2, default=999999.99, help_text="Product price"
    )
    currency = models.CharField(
        max_length=3, default="USD", help_text="ISO-4217 currency code"
    )
    is_active = models.BooleanField(
        default=True, help_text="Флаг, доступен ли продукт для покупки"
    )
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="Когда продукт был создан"
    )
    updated_at = models.DateTimeField(
        auto_now=True, help_text="Когда продукт был в последний раз обновлён"
    )
...
class CloudPaymentTransaction(models.Model):
    ...
    transaction_id = models.CharField(
        max_length=255, unique=True, help_text="CloudPayments TransactionId"
    )
    ...
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Transaction amount"
    )
    currency = models.CharField(
        max_length=3, default="RUB", help_text="ISO-4217 currency code"
    )
    ...
    ip_address = models.GenericIPAddressField(
        blank=True, null=True, help_text="IP-адрес клиента на момент оплаты"
    )
    created_at = models.DateTimeField(
    ...
    raw_data = models.JSONField(
        default=dict, blank=True, help_text="Полный JSON-ответ от CloudPayments"
    )
...
class Subscription(CoreModel):
    ...
    def save(self, *args, **kwargs):
        if not self.start_date:
            ...
            if last_subscription and (last_subscription.end_date > datetime.now()):
